...
class LinkIntegratiionTest < ActionDispatch::IntegrationTest
  test 'links index' do
    get links_path
    assert_response :ok
  end

  test 'links index paginatin' do
    (Pagy::DEFAULT[:items] + 1).times { Link.create!(url: 'https://example.org') }
    get links_path
    assert_response :ok
